...
all_result = {"name": "", "age": 0, "gender": "", "symptoms": []}
...
def predict_disease_from_symptom(symptom_list):
    ...
    if len(diseases) == 0:
        ...
        return (
            "The most likely diseases are<br><b>" + ", ".join(list(diseases)) + "</b>",
            "",
        )
    ...
    if request.args["message"] == "undefined":
        ...
        if currentState == -1:
            response.append(
                "Hi "
                + user_message
                + ", To predict your disease based on symptopms, we need some information about you. Please provide accordingly."
            )
        ...
        if currentState == 3:
            ...
            username = all_result["name"]
            response.append(
                username + ", What kind of symptoms are you currently experiencing?"
            )
        ...
        if currentState == 4:
            if "1" in user_message or "disease" in user_message:
                ...
                response.append(
                    "<b>The following disease may be causing your discomfort</b>"
                )
                ...
                response.append(
                    username
                    + ", Could you describe the symptoms you're suffering from?"
                )
                response.append("1. Check Disease")
                response.append('<a href="/diseases" target="_blank">Symptoms List</a>')
                userSession[sessionId] = userSession.get(sessionId) + 1
        ...
        if currentState == 5:
            if "1" in user_message or "disease" in user_message:
                ...
                response.append(
                    username
                    + ", What are the symptoms that you're currently dealing with?"
                )
                response.append("1. Check Disease")
                response.append('<a href="/diseases" target="_blank">Symptoms List</a>')
                userSession[sessionId] = userSession.get(sessionId) + 1
        ...
        if currentState == 6:
            if "1" in user_message or "disease" in user_message:
                ...
                username = all_result["name"]
                response.append(
                    username + ", What symptoms have you been experiencing lately?"
                )
                response.append("1. Check Disease")
                response.append('<a href="/diseases" target="_blank">Symptoms List</a>')
                userSession[sessionId] = userSession.get(sessionId) + 1
